...
use App\Exports\SignboardExport;
...
class ControllerController extends Controller
{
    ...
    {   
        ...
        ->addIndexColumn()
        // ->editColumn('tanggal_pengajuan', function ($signboard){
        //     // return date('YYYY', strtotime($signboard->tanggal_pengajuan));
        //     return date('d-M-Y', strtotime($signboard->tanggal_pengajuan));
        // })
        ->addColumn('depo', function($signboard){
            if($signboard->depo_id == null){
        ...
        ->addColumn('action', function($signboard){
            return view('signboard.controller.action', [
                'model' => $signboard,
                'url_show' => route('controller.show', $signboard->id),
                'url_edit' => route('controller.edit', $signboard->id),
    ...
    public function index()
    {
        ...
        return view('signboard.controller.index',  compact('depos'));
    }
    ...
    public function show($id)
    {
        ...
        $signboard = DesignSignboard::orderBy('urutan', 'ASC')->where([['status','=',1],['type','=',1]])->get();      
        $standing = DesignSignboard::orderBy('urutan', 'ASC')->where([['status','=',1],['type','=',2]])->get();      
        $pintu = DesignSignboard::orderBy('urutan', 'ASC')->where([['status','=',1],['type','=',3]])->get();      
        return view('signboard.controller.show', compact('model','detail','signboard','standing','pintu'));
    ...
    public function edit($id)
    {
        ...
        return view('signboard.controller.form', compact(
            'model','vendors'
        ));
    }
    public function update(Request $request, $id)
    {
        ...
        $signboard->tanggal_order = date(now());       
        $signboard->save();
    ...
    {
        // return Excel::download(new SignboardExport, 'siswa.xlsx');
        // return Excel::download(new PerbaikanExport, 'rekap_perbaikan.xlsx');
        $this->validate($request, [
            'startDate' => 'required',
            'endDate' => 'required',
            // 'filter_depo' => 'required',
            // 'filter_status' => 'required',
        ]); 

        switch ($request->input('action')) {
            case 'excel':
                if($request->filter_depo == "" && $request->filter_status == ""){
                    return (new excelExport($request->startDate, $request->endDate, "111","111" ))
                    ->download('Rekap Signboard All Depo '.date('d-M-Y', strtotime($request->startDate)).'_'.date('d-M-Y', strtotime($request->endDate)).'.xls');   
                }elseif($request->filter_depo == "" && $request->filter_status != ""){
                    return (new excelExport($request->startDate, $request->endDate, "111",$request->filter_status ))
                    ->download('Rekap Signboard All Depo '.date('d-M-Y', strtotime($request->startDate)).'_'.date('d-M-Y', strtotime($request->endDate)).'.xls');   
                }elseif($request->filter_status == "" && $request->filter_depo !=""){
                    return (new excelExport($request->startDate, $request->endDate, $request->filter_depo,"111" ))
                    ->download('Rekap Signboard '.$request->filter_depo.' '.date('d-M-Y', strtotime($request->startDate)).'_'.date('d-M-Y', strtotime($request->endDate)).'.xls');   
                }else{
                    return (new excelExport($request->startDate, $request->endDate, $request->filter_depo, $request->filter_status ))
                    ->download('Rekap Signboard '.$request->filter_depo.' '.date('d-M-Y', strtotime($request->startDate)).'_'.date('d-M-Y', strtotime($request->endDate)).'.xls');
                }
            ...
            // case 'pdf':
            //     $perbaikan = Signboard::whereBetween('created_at', [$request->startDate." 00:00:00", $request->endDate." 23:00:00"])
